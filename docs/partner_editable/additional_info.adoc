...
AWS Security Hub begins to run security checks within 2 hours after you deploy the PCI DSS and Foundational Security Best Practices controls. After the initial check, subsequent checks will occur on a periodic or change-triggered basis, depending on the control. Periodic checks run automatically within 12 hours after the most recent run. You cannot change this interval.Change-triggered checks run when the associated resource changes state. Even if the resource does not change state, the updated at time for change-triggered checks is refreshed every 18 hours. This helps to indicate that the control is still enabled.
